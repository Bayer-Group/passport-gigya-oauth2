 ...
 * Options:
 *
 *   - `apiKey`            Key used for authorize, token, user info endpoints
 *   - `authorizationURL`  URL used to obtain an authorization grant
 *   - `tokenURL`          URL used to obtain an access token
 *   - `dataCenter`        Data center used to make additional REST calls
 ...
 *         apiKey: '1_AbcD2Ef34GhiJKlm_N567o9P8QRs2tu8v4wXyZa3bcDEFgh122PIjklm_NfOP6Oqr',
...
function OAuth2Strategy(options, verify) {
  ...
  if (!verify) { throw new TypeError('OAuth2Strategy requires a verify callback'); }
    if (!options.apiKey) { throw new TypeError('OAuth2Strategy requires a apiKey option'); }
    if (!options.dataCenter) { throw new TypeError('OAuth2Strategy requires a dataCenter option'); }
  ...
  this._oauth2 = new OAuth2(options.clientID,  options.clientSecret,
    ...
    this._dataCenter = options.dataCenter;
 ...
 * Customized for Gigya /userinfo endpoint.
 *
 * @param {String} access_token
 * @param {Function} done
 * @api private
 */
OAuth2Strategy.prototype._loadUserProfile = function(access_token, done) {
  var self = this;
    ...
    var options = {
        hostname: 'fidm.' + this._dataCenter,
        port: 443,
        path: '/oidc/op/v1.0/' + this._apiKey + '/userinfo',
        method: 'GET',
        headers: {
            'Authorization': 'Bearer ' + access_token
        }
    ...
    var req = https.request(options, function(res) {
        ...
        res.on('end', function() {
            ...
            function loadIt() {
                return self.userProfile(jsonData, done);
              }
              function skipIt() {
                return done(null);
              }
              ...
              if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
                // async
                this._skipUserProfile(jsonData, function(err, skip) {
